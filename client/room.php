            ...
            <div class="panel panel-default" style="height:50px;">
				...
				<div class="showme hidden" style="position: absolute; left:-120px; top:20px;">
					...
					<?php
						$rm=mysqli_query($conn,"select * from chat_member left join `client` on client.clientid=chat_member.userid where chatroomid='$id'");
						while($rmrow=mysqli_fetch_array($rm)){
							...
							?>
							<?php echo $rmrow['clientname']; ?></span><br>
							<?php
						}
				...
				<div class="pull-right" style="margin-right:10px; margin-top:7px;">
					<?php
						if ($chatrow['userid']==$_SESSION['id']){
							...
							<a href="#delete_member" data-toggle="modal" class="btn btn-primary">Delete Member</a>
							<?php
						}
						else{
							?>
    ...
    if (!in_array($extension, ['zip', 'pdf', 'docx', 'txt'])) {
        ...
        if (move_uploaded_file($file, $destination)) {
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                copy("uploads/$filename", "../uploads/$filename");
