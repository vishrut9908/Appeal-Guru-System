...
$id=$_GET['id'];
/*this is delet query*/
mysqli_query($con,"delete from holiday where id='$id'")or die("query is incorrect...");
}
if(isset($_POST['addholiday']))
{
    $holidaydate=$_POST['holidaydate'];
    $holidayfor=$_POST['holidayfor'];
    $eid=$_SESSION['id'];
        $insertquery = "INSERT INTO holiday(holidaydate,holidayfor,employeeid) VALUES ('$holidaydate','$holidayfor','$eid')";

if(mysqli_query($con,$insertquery)) {
  echo "<script>alert('task Inserted Successfully.')</script>";
} else {
  echo "<script>alert('task Not Inserted.')</script>";
}
...
<head>
    ...
    <title>Holidays | Appeal Guru</title>
        ...
        <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0" style="background: var(--bs-orange);">
            <div class="container-fluid d-flex flex-column p-0"><a class="navbar-brand d-flex justify-content-center align-items-center sidebar-brand m-0" href="#">
                ...
                <ul class="navbar-nav text-light" id="accordionSidebar">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="holiday.php"><i class="fas fa-calendar"></i><span>Holidays</span></a></li>
...
if($stmt = $sql_con->prepare("SELECT description,employeename from tickets where priority=?")) {
$priority="high";
   $stmt->bind_param("s", $priority); 
   $stmt->execute(); 
   $stmt->bind_result($description, $employeename);
...
if($stmt = $sql_con->prepare("SELECT taskdetails, taskdatetime FROM task WHERE employeeid = ? and status=?")) {
$id=$_SESSION['id'];
$status="no";
   $stmt->bind_param("is", $id,$status); 
   $stmt->execute(); 
   $stmt->bind_result($taskdetails, $taskdatetime);
...
<?php }} ?>
                ...
                <div class="container-fluid" data-aos="fade" data-aos-duration="1500" data-aos-once="true">
                    <h3 class="text-dark mb-4">Holidays</h3>
                    <div class="card shadow card-employee">
                        <div class="card-header py-3">
                            <p class="text-primary m-0 fw-bold" style="color: var(--bs-orange);">Add Task</p>
                        </div>
                        <div class="card-body">
                            <form method="post">
                                <label for="holidaydate">Date</label>
                                <input class="form-control" type="date" name="holidaydate" required>
                                <label for="holidayfor">Holiday For:</label>
                                <textarea class="form-control" placeholder="Holiday For" name="holidayfor" required></textarea>    
                                <button class="btn btn-primary" type="submit" name="addholiday" style="background: var(--bs-orange);">Add Holiday</button></form>
                        </div>
                    </div> 
                    <div class="card shadow card-employee">
                        <div class="card-header py-3">
                            <p class="text-primary m-0 fw-bold">Holidays info</p>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-2">
                            ...
                            <div class="table-responsive table mt-2" id="dataTable-1" role="grid" aria-describedby="dataTable_info">
                                <table class="table my-0" id="dataTable">
                                    ...
                                    <tbody id="dataTable1">
                                        <tr>
                                            <?php
                                        $result=mysqli_query($con,"select * from holiday")or die ("query 1 incorrect.....");

                        while(list($id,$holidaydate,$holidayfor,$employeeid)=mysqli_fetch_array($result))
                        { 
                        echo "<tr><td>$id</td><td>$holidaydate</td><td>$holidayfor</td><td>$employeeid</td>
                        
                        <td><a href='holiday.php?id=$id&action=delete'><button style='background: var(--bs-orange);' class='btn btn-primary delete' type='button' ><span><i class='fa fa-trash'></i></span></button></a></td></tr>";
                        }?>
...
<?php
$result_db = mysqli_query($conn,"SELECT COUNT(id) FROM holiday"); 
...
for ($i=1; $i<=$total_pages; $i++) {
              $pagLink .= "<li class='page-item'><a class='page-link' href='holiday.php?page=".$i."'>".$i."</a></li>";   
}
echo $pagLink . "</ul>";  
?>
                                    </nav>
