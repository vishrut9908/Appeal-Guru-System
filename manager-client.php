...
$id=$_GET['id'];
/*this is delet query*/
mysqli_query($con,"delete from client where clientid='$id'")or die("query is incorrect...");
}
if(isset($_POST['addclient']))
{
    ...
    $assigned="no";
        $insertquery = "INSERT INTO client(clientid,clientname,contactno,address,email,password,assigned) VALUES ('$clientid','$clientname','$contactno','$address','$email','$password','$assigned')";
                ...
                <div class="container-fluid" data-aos="fade" data-aos-duration="1500" data-aos-once="true">
                    ...
                    <div class="card shadow card-employee">
                        <div class="card-header py-3">
                            <p class="text-primary m-0 fw-bold" style="color: var(--bs-orange);">Add Client</p>
                        </div>
                        <div class="card-body">
                            <form method="post">
                                <input class="form-control" type="text" placeholder="Client Id" name="clientid" required>
                                <input class="form-control" type="text" placeholder="Client Name" name="clientname" required>
                                <input class="form-control" type="tel" placeholder="Contact No." name="contactno" required>
                                ...
                                <input class="form-control" type="password" placeholder="Password" name="password" id="password" onmouseover="mouseoverPass();" onmouseout="mouseoutPass();" required>
                                <script>
...
function mouseoutPass(obj) {
  ...
  obj.type = "password";
                                ...
                                <button class="btn btn-primary" type="submit" name="addclient" style="background: var(--bs-orange);">Add Client</button></form>
                        </div>
                    </div>
                    <script>
    ...
    if (td) {
      ...
      if (txtValue.toUpperCase().indexOf(filter) > -1) {
        ...
        tr[i].style.display = "none";
                            ...
                            <div class="table-responsive table mt-2" id="dataTable-1" role="grid" aria-describedby="dataTable_info">
                                <table class="table my-0" id="dataTable">
                                    ...
                                    <tbody id="dataTable1">
                                        <tr>
                                            <?php
                                        $result=mysqli_query($con,"select * from client LIMIT $start_from, $limit")or die ("query 1 incorrect.....");

                        while(list($clientid,$clientname,$contactno,$address,$email,$assigned)=mysqli_fetch_array($result))
                        { 
                        echo "<tr><td>$clientid</td><td>$clientname</td><td>$contactno</td>
                        <td>$address</td><td>$email</td><td>$assigned</td>
                        ...
                        <td><a href='manager-client.php?id=$clientid&action=delete'><button style='background: var(--bs-orange);' class='btn btn-primary delete' type='button' ><span><i class='fa fa-trash'></i></span></button></a></td></tr>";
                        }?>
...
<?php
$result_db = mysqli_query($conn,"SELECT COUNT(clientid) FROM client"); 
...
for ($i=1; $i<=$total_pages; $i++) {
              $pagLink .= "<li class='page-item'><a class='page-link' href='manager-client.php?page=".$i."'>".$i."</a></li>";   
}
echo $pagLink . "</ul>";  
?>
                                    </nav>
