...
/*this is delet query*/
mysqli_query($con,"delete from employeeclient where id='$id'")or die("query is incorrect...");
$status = "no";
mysqli_query($conn, "UPDATE client SET assigned = '{$status}' WHERE clientid = '$clientid'");
}

if (isset($_POST['addec'])) {
    $employeeid=$_POST['teammember'];
    $clientid=$_POST['client'];
    $insertquery = "INSERT INTO employeeclient(employeeid,clientid) VALUES ('$employeeid','$clientid')";
    $status = "yes";
                $sql2 = mysqli_query($conn, "UPDATE client SET assigned = '{$status}' WHERE clientid = {$clientid}");
...
<head>
    ...
    <title>Employee Client's | Appeal Guru</title>
...
<?php }} ?>
                ...
                <div class="container-fluid" data-aos="fade" data-aos-duration="1500" data-aos-once="true">
                    <h3 class="text-dark mb-4">Employee's CLient</h3>
                    <div class="card shadow card-employee">
                        <div class="card-header py-3">
                            <p class="text-primary m-0 fw-bold">Add Employee's Client</p>
                        </div>
                        <div class="card-body">
                            <form action="" method="post">
                                <div>
                                <select class="form-control" aria-expanded="false" data-bs-toggle="dropdown" id="teammember" name="teammember" >
                                        <option class="dropdown-item" value="0">--Select Employee--</option>
                                        <?php
                                        $result=mysqli_query($con,"select employeeid,employeename,teamid from employee where type<>'manager'
...
<select class="form-control" aria-expanded="false" data-bs-toggle="dropdown" id="client" name="client" >
                                        <option class="dropdown-item" value="0">--Select Client--</option>
                                        <?php
                                        $result=mysqli_query($con,"select clientid,clientname from client
                                            ")or die ("query 1 incorrect.....");

                        while(list($clientid,$clientname)=mysqli_fetch_array($result))
                        { 
                        echo "<option class='dropdown-item' value=$clientid>$clientname</option>";
                        }?>
        
                  
                        </select>
                        <button class="btn btn-primary" type="submit" name="addec">Select</button>
</div>
        ...
        tr[i].style.display = "none";
                            ...
                            <div class="table-responsive table mt-2" id="dataTable-1" role="grid" aria-describedby="dataTable_info">
                                <table class="table my-0" id="dataTable">
                                    ...
                                    <tbody id="dataTable1">
                                        <?php
                                        $result=mysqli_query($con,"select employeeclient.id,employee.employeename,client.clientname from employeeclient inner join employee inner join client on employee.employeeid = employeeclient.employeeid and client.clientid = employeeclient.clientid LIMIT $start_from, $limit")or die ("query 1 incorrect.....");

                        while(list($id,$employeename,$clientname)=mysqli_fetch_array($result))
                        { 
                        echo "<tr><td>$id</td><td>$employeename</td><td>$clientname</td>

                                            <td><a href='manager-employee-client.php?clientid=$clientid&id=$id&action=delete'><button class='btn btn-primary delete' type='button' ><span><i class='fa fa-trash'></i></span></button></a></td></tr>";
                        }?>
                                    </tbody>
                                </table>
                            </div>
...
<?php
$result_db = mysqli_query($conn,"SELECT COUNT(id) FROM employeeclient"); 
...
for ($i=1; $i<=$total_pages; $i++) {
              $pagLink .= "<li class='page-item'><a class='page-link' href='manager-employee-client.php?page=".$i."'>".$i."</a></li>";   
}
echo $pagLink . "</ul>";  
?>
                                    </nav>
